...
func bisectionRoot(f: @escaping (Double) -> Double, a: Double, b: Double, fa: Double, fb: Double, epsilon: Double) -> Double {
    let maxIter: Int = 50
    let q = recursiveSequence(indices: 0...,
        ...
        return state1
    }, until: { s1, s2 in abs(s2.x1 - s2.x0) < 0.01 * epsilon || abs(s2.y1) < epsilon })
